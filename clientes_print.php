...
//COLOR DE TABLA
        ...
        if (!empty(isset($_REQUEST['vdesde'])) && !empty(isset($_REQUEST['vhasta']))) {
            if ($_REQUEST['opcion'] == 1) { //por codigo
                $clientes = consultas::get_datos("select * from clientes where cli_cod between ".$_REQUEST['vdesde']." and ".$_REQUEST['vhasta']);
            }else{//por nombre
                $clientes = consultas::get_datos("select * from clientes where cli_nombre between '".$_REQUEST['vdesde']."' and '".$_REQUEST['vhasta']."'");
            }
        }else{
        ...
        if (!empty($clientes)) {
            ...
            $pdf->Cell(50,5,$c['cli_ci'], 1, 0, 'C', 1);
