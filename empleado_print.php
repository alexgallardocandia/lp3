...
//COLOR DE TABLA
        ...
        if (!empty(isset($_REQUEST['vdesde'])) && !empty(isset($_REQUEST['vhasta']))) {
            if ($_REQUEST['opcion'] == 1) { //por codigo
                $empleados = consultas::get_datos("select * from v_empleado where emp_cod between ".$_REQUEST['vdesde']." and ".$_REQUEST['vhasta']);
            }else{//por descripcion
                $empleados = consultas::get_datos("select * from v_empleado where emp_nombre between '".$_REQUEST['vdesde']."' and '".$_REQUEST['vhasta']."'");
            }
        }else{
        ...
        if (!empty($empleados)) {
            ...
            $pdf->Cell(50,5,$e['car_cod']."-".$e['car_descri'], 1, 0, 'C', 1);
            $pdf->Cell(50,5,$e['emp_nombre'], 1, 0, 'L', 1);
            $pdf->cell(50,5,$e['emp_apellido'],1, 0,'L',1);
            $pdf->cell(50,5,$e['emp_tel'],1, 0,'C',1);
            $pdf->cell(50,5,$e['emp_direcc'],1, 0,'C',1);
            $pdf->Ln();
        }
        }else{
            $pdf->Cell(0, 0, "No se han registrado empleados...", 1, 0, 'L', 1);
        }
