...
if (!empty(isset($_REQUEST['opcion']))) {
    switch ($_REQUEST['opcion']) {
        case 1: //por fecha
            $cabeceras = consultas::get_datos("select * from v_orden "
                    . "where orden_fecha::date between '".$_REQUEST['vdesde']."' and '".$_REQUEST['vhasta']."' order by orden_cod desc");
            break;
        ...
        case 3: //por articulo
            $cabeceras = consultas::get_datos("select * from v_orden "
                    . "where orden_cod in(select orden_cod from detalle_compra where art_cod in(".$_REQUEST['varticulo']."))");
            break;
        case 4: //por empleado
            $cabeceras = consultas::get_datos("select * from v_orden "
    ...
    $cabeceras = consultas::get_datos("select * from v_orden where orden_cod=".$_REQUEST['vorden_cod']);
}
if (!empty($cabeceras)) {
    foreach ($cabeceras as $cabecera) {
        ...
        $pdf->Cell(80,2,'COMPRA N°: '.$cabecera['orden_cod'],0,1);
        ...
        $detalles = consultas::get_datos("select * from v_detalle_orden where orden_cod=".$cabecera['orden_cod']);
        if (!empty($detalles)) {
           ...
           foreach ($detalles as $det) {
               ...
               $pdf->Cell(20,5, number_format($det['orden_precio'],0,',','.'),1,0,'C',1);
           ...
           $pdf->Cell(60,5,number_format($cabeceras[0]['orden_total'],0,',','.'),1,0,'C',1);
               $pdf->Ln();
               $pdf->Ln();
        }else{
            $pdf->Cell(135,5,'La orden no tiene detalles cargados',0,'L',1);
        }
    }
